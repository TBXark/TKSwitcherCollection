...
open class TKSmileSwitch: TKBaseSwitch {
    ...
    override internal func setUpView() {
        super.setUpView()
        // 使用 draw(_:) 绘制轨道, 必须声明为透明背景, 否则 opaque 图层会渲染成黑底
        backgroundColor = .clear
        isOpaque = false
        let side = bounds.height
        let x: CGFloat = isOn ? 0 : bounds.width - side
        let face = TKSmileFaceView(frame: CGRect(x: x, y: 0, width: side, height: side))
        face.faceType = isOn ? TKSmileFaceView.FaceType.happy : .sad
        addSubview(face)
        smileFace = face
    }
    ...
    override open func draw(_ rect: CGRect) {
        ...
        let path = UIBezierPath(
            roundedRect: rect.insetBy(dx: lineWidth, dy: lineWidth), cornerRadius: rect.width / 2)
    ...
    override func animateValueChange(_ value: Bool, duration: Double) {
        guard let smileFace = smileFace else {
            return
        }
        let x: CGFloat = value ? 0 : (bounds.width - bounds.height)
        let frame = CGRect(x: x, y: 0, width: bounds.height, height: bounds.height)
        smileFace.faceType = value ? TKSmileFaceView.FaceType.happy : .sad
        smileFace.rotation(animationDuration, count: 2, clockwise: !isOn)
        UIView.animate(
            withDuration: duration,
            animations: { () -> Void in
                smileFace.frame = frame
            },
            completion: { (complete) -> Void in
                if complete {
                    smileFace.eyeWinkAnimate(duration / 2)
                }
            })
    }
}
...
private class TKSmileFaceView: UIView {
    ...
    var mouthPath: UIBezierPath {
        ...
        path.addCurve(
            to: CGPoint(x: 70 * sizeScale, y: 40 * sizeScale),
    ...
    private func setupLayers() {
        ...
        let facePath = UIBezierPath(
            ovalIn: CGRect(x: 0, y: 0, width: 100 * sizeScale, height: 100 * sizeScale))
        let eyePath = UIBezierPath(
            ovalIn: CGRect(x: 0, y: 0, width: 20 * sizeScale, height: 20 * sizeScale))
        ...
        leftEye.frame = CGRect(
            x: 20 * sizeScale, y: 28 * sizeScale, width: 10 * sizeScale, height: 10 * sizeScale)
        leftEye.path = eyePath.cgPath
        leftEye.fillColor = UIColor.white.cgColor
        layer.addSublayer(leftEye)

        rightEye.frame = CGRect(
            x: 60 * sizeScale, y: 28 * sizeScale, width: 10 * sizeScale, height: 10 * sizeScale)
    ...
    func rotation(_ duration: Double, count: Int, clockwise: Bool) {
        ...
        rotationTransformAnim.values = [0, CGFloat.pi * CGFloat(count) * (clockwise ? 1 : -1)]
